"""xAI Grok LLM call and tools detector."""
...
class GrokDetector:
    """Detects xAI Grok completions and tool bindings."""
    ...
    def detect_call(self, call: ast.Call, file_path: str, lineno: int) -> Optional[LLMCallSite]:
        ...
        if ("chat.completions.create" in func_str) and any(k in func_str.lower() for k in ("xai", "grok")):
            model_name = None
            tools_list: List[str] = []
